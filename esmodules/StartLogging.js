...
async function recordRoll({ id, type, roller, vs = null, d20, needs, dos }) {
  ...
  update[`flags.confirm-bias.sessions.${SESSION_UUID}.${rollerId}.${d20}.${dos}.${type}.${id}`] = { vs: vsId, needed: needs };
...
Hooks.on('createChatMessage', async (message, options, id) => {
  ...
  // console.log('createChatMessage', { message, roll, type });
  ...
  recordRoll({ id: message.id, type, roller, vs, d20, dos, needs, isReroll });
});

Hooks.on('updateChatMessage', async (message, delta, options, id) => {
  const toolbelt = delta?.flags['pf2e-toolbelt'];
  // console.log('updateChatMessage', { message, delta });
  ...
  if (toolbelt) {
    ...
    for (const tokenId in saves) {
      ...
      recordRoll({ id: message.id, type: 'saving-throw', roller, vs, d20, dos, needs, isReroll });
    }
    return;
  }
